...
async function run() {
  const { data: events, error } = await supabase.from('orchestrator_events').select('*').eq('status', 'pending').eq('event_type', 'task_activation');
  if (error) console.error(error); else console.log(JSON.stringify(events));
}
run();
